...
"""this section starts the construction of the curve VAE"""
...
class CurveVAE(nn.Module):
    def __init__(self, latent_dim: int = 32, num_points: int = 100):
        super().__init__()
        self.encoder = CurveEncoder(latent_dim)
        self.decoder = CurveDecoder(latent_dim, num_points)
...
if __name__ == "__main__":
    ...
    vae.load_state_dict(torch.load(pointcloud_checkpoint_path, map_location="cpu"))


    vae = CurveVAE(num_points=320, latent_dim=64)
    curve_checkpoint_path = os.path.join(checkpoint_dir, 'curve_vae_state.pth')
    vae.load_state_dict(torch.load(curve_checkpoint_path, map_location="cpu"))
