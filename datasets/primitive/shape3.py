...
class Shapes3DatasetUnwrapped(FilterableDatasetUnwrapped):
    def __init__(self, dataset_size):
        self._data = gen_shapes3_dataset(dataset_size)
    ...
    def _get_metainfo(self, index):
        return {
            'query': self._data['questions'][index],
            'program': self._data['programs'][index],
            'answer': bool(self._data['answers'][index]),
            'question_length': len(self._data['questions'][index].split())
        }

    def __getitem__(self, index):
        return {
            'image': _to_image(self._data['images'][index]),
            'query': self._data['questions'][index],
            'program': self._data['programs'][index],
            'answer': bool(self._data['answers'][index]),
            'grounding' : {"image" : _to_image(self._data['images'][index])},
        }
