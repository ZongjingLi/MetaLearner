...
class RopeEngine(object):
    ...
    def step(self):
        for impulse, ball in zip(self.actions, self.balls):
            ball.apply_impulse_at_local_point(impulse=list(impulse), point=(0, 0))
        self.space.step(self.dt)
...
class BoxEngine(object):
    ...
    def get_vis(self, states):
        ...
        buf = np.sum(buf.astype(int), axis=0)
        vis = buf == 0
        return vis
...
def test_gen_Box(info):
    ...
    for i in bar(range(n_rollout)):
        ...
        vis = np.zeros((time_step, n_particle), dtype=bool)
