...
def condition_assign(x, y):
        ...
        # Check if this predicate needs object indexing
        needs_idx = x.get("idx") is not None and len(x.get("from", "").split("-")) > 1
        
        return {**x, "end": [{
            "x": x.get('idx', None) if needs_idx else None,  # Only use idx if needed
            "v": y["end"],
            "c": torch.tensor(infinity, device=device),
            "to": x["from"]
        }]}
