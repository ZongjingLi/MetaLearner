...
def tvector(element_type, dimension=None):
    """Create a vector type with specified element type and optional dimension.
    
    Args:
        element_type: The type of vector elements (e.g., treal, tint)
        dimension: Optional fixed dimension. If None, dimension is not constrained.
    """
    if dimension is not None:
        return TypeConstructor("vector", [element_type, TypeConstructor(str(dimension), [])])
    return TypeConstructor("vector", [element_type])

# Base vector types with common dimensions
def tvector2(element_type):
    """Create a 2D vector type with specified element type."""
    return tvector(element_type, 2)

def tvector3(element_type):
    """Create a 3D vector type with specified element type."""
    return tvector(element_type, 3)

def tvector4(element_type):
    """Create a 4D vector type with specified element type."""
    return tvector(element_type, 4)
...
def guess_type(xs):
    ...
    elif all(isinstance(x, (list, tuple)) and len(x) == len(xs[0]) for x in xs):
        # Handle vectors - checking if all elements have same length
        dimension = len(xs[0])
        element_values = [elem for x in xs for elem in x]
        element_type = guess_type(element_values)
        return tvector(element_type, dimension)
    else:
